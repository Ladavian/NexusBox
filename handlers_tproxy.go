...
func loadTproxyDstExceptions() []string {
    ...
    data, err := os.ReadFile(nexusboxConfigFile)
    if err != nil {
...
func saveTproxyDstExceptionsLocked(dst []string) error {
    ...
    return os.WriteFile(nexusboxConfigFile, newData, 0644)
}
...
// loadTproxyProxyLocal 从 nexusbox.json 读取 tproxy_proxy_local 字段，默认 true
...
func saveTproxyProxyLocalLocked(enabled bool) error {
	...
	dir := filepath.Dir(nexusboxConfigFile)
...
func enableTProxyRules(port int) error {
	...
	// 4. 绕过私有网段
	runCmd("nft", "add", "set", "ip", "nexusbox_tproxy", "private_ips", "{ type ipv4_addr; flags interval; }")
	...
	for _, ip := range bypassIPs {
		runCmd("nft", "add", "element", "ip", "nexusbox_tproxy", "private_ips", "{", ip, "}")
	}

	// 5. 创建链
	runCmd("nft", "add", "chain", "ip", "nexusbox_tproxy", "prerouting", "{ type filter hook prerouting priority mangle; policy accept; }")
	runCmd("nft", "add", "chain", "ip", "nexusbox_tproxy", "output", "{ type route hook output priority mangle; policy accept; }")
	runCmd("nft", "add", "chain", "ip", "nexusbox_tproxy", "dstnat", "{ type nat hook prerouting priority -100; policy accept; }")
	runCmd("nft", "add", "chain", "ip", "nexusbox_tproxy", "nat_output", "{ type nat hook output priority -100; policy accept; }")

	// 6. 私有IP绕过
	runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "prerouting", "fib", "daddr", "type", "local", "return")
	runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "prerouting", "ip", "daddr", "@private_ips", "return")
	runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "output", "ip", "daddr", "@private_ips", "return")
	...
	for _, rule := range dstExceptions {
		...
		if typ == "ip" {
			cidr := ipNet.String()
			// TProxy 劫持
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "prerouting", "ip", "daddr", cidr, "return")
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "output", "ip", "daddr", cidr, "return")
			// DNS 重定向
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "dstnat", "ip", "daddr", cidr, "return")
			if tproxyProxyLocal {
				runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "nat_output", "ip", "daddr", cidr, "return")
			}
		} else if typ == "port" {
			// 将集合作为一个完整的字符串参数
			protoExpr := "{tcp, udp}"
			...
			// TProxy 劫持
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "prerouting", "meta", "l4proto", protoExpr, "th", "dport", strconv.Itoa(portVal), "return")
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "output", "meta", "l4proto", protoExpr, "th", "dport", strconv.Itoa(portVal), "return")
			// DNS 重定向
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "dstnat", "meta", "l4proto", protoExpr, "th", "dport", strconv.Itoa(portVal), "return")
			if tproxyProxyLocal {
				runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "nat_output", "meta", "l4proto", protoExpr, "th", "dport", strconv.Itoa(portVal), "return")
			}
		}
	}
	...
	for _, rule := range srcExceptions {
		...
		if _, ipNet, err := net.ParseCIDR(rule); err == nil {
			cidr := ipNet.String()
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "prerouting", "ip", "saddr", cidr, "return")
			runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "dstnat", "ip", "saddr", cidr, "return")
	...
	// 8. TProxy 劫持规则
	// 使用集合字符串 "{tcp,udp}"
	runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "prerouting", "meta", "l4proto", "{tcp,udp}", "tproxy", "to", fmt.Sprintf(":%d", port), "meta", "mark", "set", "1", "accept")
	runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "output", "meta", "mark", "0xff", "return")
	runCmd("nft", "add", "rule", "ip", "nexusbox_tproxy", "output", "socket", "mark", "0xff", "return")
	if tproxyProxyLocal {
...
func disableTProxyRules() {
	exec.Command("nft", "delete", "table", "ip", "nexusbox_tproxy").Run()
