...
export const useGlobalStore = defineStore('global', () => {
  const startPage = ref<string>(localStorage.getItem('nexusbox-start-page') || 'last')

  const getInitialTab = () => {
    const sp = startPage.value
    return sp === 'last' ? (localStorage.getItem('nexusbox-active-tab') || 'overview') : sp
  }

  const activeTab = ref<string>(getInitialTab())
  const isSidebarCollapsed = ref<boolean>(localStorage.getItem('nexusbox-sidebar-collapsed') === 'true')
  const theme = ref<string>(localStorage.getItem('nexusbox-theme') || 'light')
  ...
  watch(activeTab, (newTab) => {
    localStorage.setItem('nexusbox-active-tab', newTab)
  })

  watch(isSidebarCollapsed, (val) => {
    localStorage.setItem('nexusbox-sidebar-collapsed', val ? 'true' : 'false')
  })

  watch(startPage, (newVal) => {
    localStorage.setItem('nexusbox-start-page', newVal)
  })
