...
import { CloudDownloadOutline, OptionsOutline, HardwareChipOutline, ShieldCheckmarkOutline, BuildOutline, SearchOutline, SyncOutline, ColorPaletteOutline, SettingsOutline, InformationCircleOutline, DocumentTextOutline, CodeSlashOutline } from '@vicons/ionicons5'
...
const saveAuthConfig = async () => {
  if (!authUsername.value || !authPassword.value) {
    globalStore.showToast(t('login.empty_fields'), 'warning')
    return
  }
  ...
  try {
    ...
    if (resp.ok) {
      globalStore.showToast(t('config.auth_save_success'), 'success')
      authPassword.value = ''
    } else {
      globalStore.showToast(t('common.operation_failed'), 'error')
    }
...
const loadYamlConfig = async () => {
  ...
  try {
    const resp = await apiFetch('/configs/raw')
    if (resp.ok) {
      yamlContent.value = await resp.text()
      yamlLoaded.value = true
    } else {
      globalStore.showToast(t('config.yaml_load_failed'), 'error')
    }
  } catch (e) {
    globalStore.showToast(`${t('common.error')}: ${(e as Error).message}`, 'error')
  } finally {
...
const saveYamlConfig = async () => {
  if (!yamlContent.value.trim()) {
    globalStore.showToast(t('config.yaml_empty_warning'), 'warning')
    return
  }
  const confirmed = await globalStore.showConfirm({
    title: t('common.warning'),
    message: t('config.yaml_save_confirm'),
    type: 'warning'
  })
  if (!confirmed) return
  ...
  try {
    ...
    const data = await resp.json()
    if (resp.ok && data.status === 'ok') {
      globalStore.showToast(t('config.yaml_save_success'), 'success')
    } else if (data.status === 'warning') {
      globalStore.showToast(data.message || t('config.yaml_save_warning'), 'warning')
    } else {
      globalStore.showToast(t('config.yaml_save_failed'), 'error')
    }
  } catch (e) {
    globalStore.showToast(`${t('common.error')}: ${(e as Error).message}`, 'error')
  } finally {
  ...
  <div class="flex flex-col flex-1 min-h-0 gap-4 h-full">
    ...
    <div v-else class="flex-1 min-h-0 overflow-y-auto glass-medium shadow-none rounded-xl border border-slate-200/50 dark:border-slate-800/50 p-6">
      <div class="grid grid-cols-1 gap-6 items-start w-full animate-[fadeIn_0.25s_ease-out]"
        ...
        <div
          ...
          <div class="space-y-4 flex-1">
            ...
            <div class="pb-3 border-b border-slate-100 dark:border-slate-800/60">
              <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2 block">{{ t('config.auth_settings') }}</label>
              <div class="flex flex-col gap-2">
                <input type="text" v-model="authUsername" :placeholder="t('login.username')"
                  class="px-3 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-accent outline-none w-full" />
                ...
                <button @click="saveAuthConfig" :disabled="authSaving"
                  class="py-1.5 bg-accent hover:bg-accent-hover text-white text-xs font-semibold rounded-lg transition-all flex items-center justify-center gap-1 disabled:opacity-50">
                  <div v-if="authSaving" class="w-3 h-3 border border-white/30 !border-t-white rounded-full animate-spin"></div>
                  {{ t('common.save') }}
                </button>
              </div>
            </div>
        ...
        <!-- 7. YAML 配置文件编辑器 -->
        <div class="live-card bg-slate-50/50 dark:bg-slate-900/30 rounded-xl border border-slate-200/40 dark:border-slate-800/40 hover:border-slate-300/80 dark:hover:border-slate-700/80 duration-300 transition-all col-span-full flex flex-col">
          <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
            ...
            <div class="flex items-center gap-1.5">
              <button @click="loadYamlConfig" :disabled="yamlLoading"
                class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-accent/10 text-accent border border-accent/20 hover:bg-accent hover:text-white transition-all disabled:opacity-50 flex items-center gap-1">
                <SyncOutline v-if="yamlLoading" class="w-3 h-3 animate-spin" />
                {{ yamlLoading ? '...' : t('config.yaml_load') }}
              </button>
