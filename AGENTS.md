...
`NexusBox` 是一个轻量级、无冗余的 Mihomo 内核管理面板与订阅生成系统。它采用**前后端不分离**的架构设计：
- **后端 (Go)**：使用 Go 1.26 标准库（仅引入 `gorilla/websocket` 作为唯一外部依赖）。后端托管在 Unix Socket (`/var/apps/NexusBox/target/app.sock`) 上，对外通过前端反向代理暴露，内嵌了前端的所有静态资源。
- **前端双版本并存与条件编译**：
...
3. **通信中转代理 (Bridge)**：由于 Mihomo 运行在本地 Unix Socket 上，NexusBox 后端作为前端与本地内核之间的“双向桥梁”，代理所有的 HTTP API 请求与 WebSocket 数据流（流量、内存、连接、日志等），并自动附加 `Bearer Token` 认证。
...
### 3.1 统一路由前缀 (BASE_URL)
所有的请求均有统一的基本路径前缀：`baseURL = "/app/NexusBox"`。
...
> 所有路由均挂载在 `baseURL = "/app/NexusBox"` 之下，如 `/app/NexusBox/core/status`。
