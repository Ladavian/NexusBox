...
func handleSelfUpdate(w http.ResponseWriter, r *http.Request) {
	...
	// 确定目标路径
	targetPath := filepath.Join(nexusboxBinDir, "nexusbox")
	backupDir := filepath.Join(nexusboxBinDir, "nexusbox-backup")
	...
	archMap := map[string]string{
		"amd64":  "nexusbox-amd64",
		"arm64":  "nexusbox-arm64",
		"arm":    "nexusbox-arm64",
		"x86":    "nexusbox-amd64",
		"x86_64": "nexusbox-amd64",
	}
	...
	// 创建临时文件
	tmpFile, err := os.CreateTemp("", "nexusbox-update-*")
	if err != nil {
	...
	if _, err := os.Stat(targetPath); err == nil {
		backupName := filepath.Join(backupDir, "nexusbox")
